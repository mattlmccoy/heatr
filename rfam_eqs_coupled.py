...
# v2.1.0 WIRED the three adopted decisions into the solve-mode production
#   path: the dense-if-and-only-if-in-bounds (J_asym) stop rule as the default
#   read state of a solve-mode run, the method of moving asymptotes as the
#   optimizer default on constrained objectives, and the 1/|g0| objective
#   rescale at solve start. BEHAVIOUR CHANGE: a default fgm_solve run now
#   delivers at a later stop; fgm_solve.stop_rule "j_phi" restores the v2.0.x
#   read state. The engine's own forward physics is untouched.
...
ENGINE_VERSION_NAME = "HEATR 2D v2.1"
