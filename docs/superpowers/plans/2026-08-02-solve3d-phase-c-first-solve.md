...
- [x] **DONE.** `design_chain.py`: EXPLICIT normalized-convolution filter matrix (volume-weighted Gaussian, 1.0 mm physical radius, 3-sigma truncation) chosen over a Helmholtz filter precisely because it is NOT self-adjoint, so its transpose gate tests something -- measured asymmetry 0.1322. Filter transpose 1.294e-15, composed transpose 1.294e-15 (beta 0) and 3.599e-16 (beta 1), against the 1e-13 class. Projection fixes 0/0.5/1 exactly, monotone, derivative FD-checked at beta 1 and 16, and beta<=0 is BIT-identical to the filter alone. Kernel measured std 0.9087 mm against the 1.0 mm radius (3-sigma truncation), 52 mean neighbours, row sums to 6.7e-16. Composed dJ/dv FD gate on the small Phase B case, asymmetric objective: 4/4 probes at the PREFERRED 1e-6 at BOTH beta 0 and beta 1. **13 passed.**
