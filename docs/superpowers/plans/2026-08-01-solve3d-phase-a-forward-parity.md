...
- [ ] Write `solve3d/tests/test_forward_analytic.py::test_env_imports` asserting `solve3d.forward` imports dolfinx (via jit_fix) and exposes `ForwardParams` / `run_forward`. Run: fails (module missing).
- [ ] Create the scaffold with a `ForwardParams` frozen dataclass mirroring the relevant `heatr3d.Params` fields (material props, drive, `eqs_update_interval_s`, `sigma_temp_coeff_per_K`, `sigma_density_coeff`, `sigma_ref_temp_c`, `eqs_resolve_drift_rtol`, `phase_update` fixed to enthalpy).
- [ ] Run test: passes. Commit.

### Task 1: Pre-register parity tolerances from heatr3d self-spread

- [ ] Script `solve3d/cases.py::measure_self_spread()`: run heatr3d (enthalpy, coupled defaults OFF) on the extruded circle anchor at n=64 and n=96; record the relative spread of t90, part-mean heating curve (rel-L2), and std(T) at heating-peak.
- [ ] Write the tolerances into `solve3d/results/parity_tolerances.json` as 1.5x the measured spread, with the raw numbers alongside. These are the FROZEN Phase A gates; later tasks read this file.
- [ ] Commit with the JSON.

### Task 2: EQS forward lift

- [ ] Failing test: `test_forward_parity.py::test_eqs_qrf_pattern` — dolfinx Q_rf on the extruded circle vs heatr3d masked-gradient Q_rf (resampled to the voxel grid): pattern rel-L2 < 0.05 (the D1 Task-4 gate, already proven achievable) and total power matched to 1e-9 after renormalization.
- [ ] Implement by lifting `eqs_common` conventions into `solve3d/forward.py` (corrected-Q only).
- [ ] Run: pass. Commit.

### Task 3: Enthalpy march — analytic benchmarks (S1 ports)

- [ ] Failing tests (three, from `test_heatr3d_s1.py` analogs): (a) latent plateau — uniform heating pins the melt-range plateau at the correct enthalpy budget; (b) Fourier decay — zero-source decay rate matches the analytic eigenvalue within 2%; (c) energy audit — |residual_frac| < 1e-2 on a heat+melt case.
- [ ] Implement the transient enthalpy march (piecewise-linear H(T) inversion ported from `heatr3d.enthalpy_from_T`/`T_from_enthalpy`; implicit or CFL-guarded explicit stepping — agent's choice, justified in the report).
- [ ] Run: pass. Commit.

### Task 4: Coupled-forward parity on the anchor cases

- [ ] Failing test: `test_forward_parity.py::test_anchor_parity` — for extruded circle AND extruded square: t90, part-mean heating curve rel-L2, and heating-peak std(T) within the Task-1 frozen tolerances, run with `eqs_update_interval_s > 0` and a nonzero `sigma_temp_coeff_per_K` (use −0.002 /K, the S4 re-score's best-behaved value, |a| well inside the 0.0044 validity bound) so re-solve semantics are exercised, plus the defaults-off case.
- [ ] Also assert `n_eqs_solves` parity semantics: same re-solve count as heatr3d for the same interval and horizon.
- [ ] Implement the re-solve loop (port `apply_sigma_coupling` semantics + fixed-power renormalization).
- [ ] Run: pass. Emit `solve3d/results/phase_a_gate.json` with every number. Commit.

### Task 5: Gate report

- [ ] `solve3d/PHASE_A_REPORT.md`: gates table quoted from the JSONs, cost table (wall time per solve/march vs heatr3d), honest deviations, what is NOT covered (no adjoint, no objective, no regularization — those are Phases B+). Commit.
