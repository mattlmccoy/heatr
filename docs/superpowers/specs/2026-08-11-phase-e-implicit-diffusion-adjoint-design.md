...
`dt < dt_stable ~ cell_size²·ρc_p/k`. Fine on the coarse square/cube (CFL-stable);
on the fine Tamper mesh (54495 cells, min-cell `dt_stable≈3.3e-3 s`) `dt=0.5` is
~150× above CFL and **54% of cells are explicit-unstable**. The forward survives
via clamps (temp cap, `nan_to_num`); the linearized REVERSE co-state has no
clamps, amplifies ~1.36×/step, overflows to Inf by reverse-step ~1300, then
`Inf×0` (a zeroed subgradient mask) → NaN. Drive-independent (0.7× and 1.2×
...
- **Memory:** the reverse sweep stores a cache per substep (`keep_cache=True`,
  `for c in reversed(caches)`); 470k caches × ~20 nodal arrays × 20k nodes ≈
  hundreds of TB. Would need checkpointing (recompute-during-reverse).
- **Compute:** 168× more substeps → the ~15 h Tamper solve becomes months.
...
`T_in` (conductivity `k(T_in)`, `ρc_p(T_in)`, phase state) so `T_new` appears
...
3. Wire into `_march`/`dks_peak_ds`, FD-gate on the Tamper. 4. Fidelity decision
   (§4) wired + `march_fidelity_check` re-baselined. 5. Re-run a coarse B-stage to
