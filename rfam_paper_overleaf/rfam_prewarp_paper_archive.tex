...
%% 5p = two-column journal layout; times = Times Roman font.
%% natbib is included by elsarticle.  DOIs rendered by elsarticle-num.
...
This paper presents and evaluates three complementary strategies
implemented in HEATR (High-frequency Electrothermal Additive Thermal
Resolver), a 2D EQS--thermal forward model developed for RFAM: (1) an
edge placement error (EPE)-based iterative geometry prewarp algorithm,
inspired by inverse lithography technology (ILT) from semiconductor
photomask design; (2) spike assist features, analogous to sub-resolution
assist features (SRAFs) in optical lithography, placed at the
under-heated parallel faces to concentrate field energy locally; and
(3) a time-based turntable rotation schedule that averages the
anisotropic exposure by rotating the part in the fixed RF field.
...
\begin{enumerate}
\item HEATR (High-frequency Electrothermal Additive Thermal Resolver):
      a validated 2D EQS + thermal + densification forward model for
...
\subsection{HEATR Forward Model}
\label{sec:eqs_model}

All simulation results in this paper are produced by HEATR
(High-frequency Electrothermal Additive Thermal Resolver), an
open-source 2D EQS--thermal--densification solver implemented in Python.
HEATR consists of three coupled modules: an EQS electric-field solver,
a transient thermal solver with apparent-heat-capacity phase change, and
a density ODE integrator.  Each module is described below.
...
\SI{25}{\celsius}$; all other boundaries are adiabatic.  An explicit Forward Euler scheme is used with automatic sub-stepping
(CFL criterion) and a maximum outer step of $\Delta t = \SI{0.5}{\second}$;
five outer steps occur per EQS solve interval (\SI{2.5}{\second}).
...
tension, analogous to Frenkel's viscous sintering
model~\cite{Frenkel1945,Zarringhalam2009}):
\begin{equation}
...
These values are consistent with measured crystallization and melt
kinetics for PA12 reported in~\cite{zhao2018crystallization,peyre2015sls,Zarringhalam2009}.  The initial density is $\rhorel(t=0) = 0.55$
uniformly throughout the doped region, matching the bulk poured density
of graphite-doped PA12 powder~\cite{verbelen2016characterization}.
...
\begin{figure*}[!t]
  \centering
  \includegraphics[width=\textwidth]{fig_primitive_sweep.png}
...
the prewarped square polygon, and \cref{fig:spike_coopt} shows
the optimized spiked geometry alongside the resulting density field.
The spike profile (\cref{eq:spike_profile})
...
\begin{figure}[H]
  \centering
  \includegraphics[width=\columnwidth]{fig_spike_coopt.png}
...
\begin{figure*}[!t]
  \centering
  \includegraphics[width=\textwidth]{fig_uniformity_comparison.png}
...
%% ---------------------------------------------------------------
\clearpage
\section{L-Shape: Static Baseline, Turntable, and Exposure Optimizer}
\label{sec:lshape}
%% ---------------------------------------------------------------
...
density, confirming that geometric prewarp or spike features are
...
\subsection{Exposure Optimizer Result}

The exposure-time optimizer seeks the power delivery schedule that
maximises density uniformity subject to peak-temperature and total-energy
constraints.  \Cref{fig:lshape_opt} presents the optimised result for
the same L-shape geometry and 8-minute budget.  The optimizer converges
to a schedule that closely matches the fixed-power single-orientation
baseline, demonstrating that temporal power modulation alone cannot
overcome geometric coupling anisotropy: the under-heated parallel faces
receive less $\Qrf$ at any power level, so adjusting the power--time
profile does not redistribute energy between face orientations.  This
confirms that spatial strategies---turntable rotation or geometric
prewarp---are required for substantial uniformity improvement on
asymmetric shapes.

\begin{figure*}[!tp]
  \centering
  \includegraphics[width=\textwidth]{fig_lshape_opt_report.png}
  \caption{L-shape exposure-time optimizer output.
           \textit{Top:} time-series of mean part temperature, peak
           temperature $T_{\max}$, mean melt fraction $\bar{\phimelt}$,
           and mean relative density $\bar{\rhorel}$.  Two candidate
           stopping times are identified: the \emph{max-density ceiling}
           ($t = \SI{337}{\second}$; $T_{\max} < \SI{250}{\celsius}$,
           $\bar{\phimelt} = 0.66$, $\bar{\rhorel} = 0.747$) and the
           \emph{melt criterion} ($t = \SI{422}{\second}$; $\bar{\phimelt}
           \geq 0.75$, $\bar{\rhorel} = 0.820$).  The recommended stopping
           time is 422~s (7~min 2~s), with only 84~s of thermal margin
           remaining before the peak-temperature ceiling.
           \textit{Bottom rows:} temperature, melt fraction, and relative
           density fields at each stopping time.  The optimizer identifies
           temporal stopping criteria but cannot redistribute energy between
           geometric face orientations; turntable or prewarp strategies are
           required for uniformity improvement (\Cref{fig:lshape_tt}).}
